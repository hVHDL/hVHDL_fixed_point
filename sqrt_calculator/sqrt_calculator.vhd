...
-- fixed_dsp can be any word length >= sqrt_word_length (e.g. a real
-- 32x32 hard multiplier, wider than lut_sqrt_pkg's own tables) : the
-- a/b/c operands are resized up to whatever width fixed_dsp_in actually
-- has before use, and the result is resized back down to
-- sqrt_word_length once read back, which is exact regardless of the
...
begin
    ...
    dsp_interpolated <= resize(shift_right(fixed_dsp_out.result, sqrt_fraction_width), sqrt_word_length);

    sqrt_calculator_out.ready_with_1 <= fixed_dsp_out.ready_with_1;
